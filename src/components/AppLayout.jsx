...
export default function AppLayout({ children, collapsibleSidebar = false }) {
  ...
  return (
    <div className="bg-surface font-body text-on-surface min-h-screen flex">
      {/* Sidebar toggle button (visible when collapsible and sidebar hidden) */}
      {collapsibleSidebar && sidebarHidden && (
        <button
          onClick={() => setSidebarHidden(false)}
          className="fixed top-4 left-4 z-50 hidden md:flex w-10 h-10 rounded-xl bg-white/90 backdrop-blur-xl border border-slate-200/60 items-center justify-center shadow-lg hover:bg-primary/10 transition-all"
          title="Show sidebar"
        >
      ...
      <aside className={`w-20 bg-white/80 backdrop-blur-xl border-r border-slate-200/60 hidden md:flex flex-col items-center py-4 gap-1 sticky top-0 h-screen z-40 shrink-0 transition-all duration-300 ${collapsibleSidebar && sidebarHidden ? '!hidden' : ''}`}>
        {/* Aurea3D Logo */}
        <Link to="/" className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center hover:bg-primary/20 transition-colors" title="Aurea3D">
        ...
        <div className="w-8 h-px bg-slate-200 mb-2 mt-1"></div>
      ...
      {/* Mobile Top Header (hidden in editor) */}
      {!collapsibleSidebar && (
      ...
      {/* Main Content */}
      <div className={`flex-1 flex flex-col min-h-screen ${collapsibleSidebar ? '' : 'mt-16 md:mt-0 pb-24 md:pb-0'}`}>
        {/* Top Tab Bar (hidden in editor) */}
        {!collapsibleSidebar && (
